...
from mmdet.core import ccpd_classes

label_ids = {name: i for i, name in enumerate(ccpd_classes())}
...
def cvt_annotations(devkit_path, split, out_file):
    ...
    filelist = osp.join(devkit_path,
                        f'VOC/ImageSets/Main/{split}.txt')
    ...
    xml_paths = [
        osp.join(devkit_path, f'VOC/Annotations/{img_name}.xml')
...
def cvt_to_coco_json(annotations):
    ...
    for category_id, name in enumerate(ccpd_classes()):
        category_item = dict()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Convert CCPD VOC annotations to mmdetection format')
    parser.add_argument('devkit_path', help='ccpd voc devkit path')
